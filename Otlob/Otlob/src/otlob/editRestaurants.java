...
public editRestaurants()
{
    super("window");
        ...
        try {
            setContentPane(new JLabel(new ImageIcon(ImageIO.read(new File("C:\\Users\\Amir Abu Gabal\\Desktop\\Otlob\\otlobImage2.jpg")))));
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        for(int i=0;i<restaurants.toArray().length;i++){
           Fond  = new JTextField();
           Fond.setText(restaurants.get(i).toString());
           System.out.println(restaurants.get(i).toString());
           
           Fond.setLocation(250,300+x);
           Fond.setSize(400,20);
        }
        /*
        for (int i = 0; i < restaurants.toArray().length; i++) {
        list=new JList(restaurants.get(i).toString());
        list.setLocation(310,310);
        list.setSize(400,20);
    }*/
        
        
        //rests.setSize(150, 20);
        //rests.setLocation(115,146);
