...
public selectedRestUser()
{
        ...
        try {
            setContentPane(new JLabel(new ImageIcon(ImageIO.read(new File("D:\\Otlob (1)\\Otlob\\otlobImage2.jpg")))));
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        for (int i = 0; i <items.toArray().length; i++) {
            if(myChoice.equals(items.get(i).getRestID())){
                ...
                //itemArray[j]=items.get(i).toString();
                //j++;
                }
        }
        
        
        JComboBox comboBox = new JComboBox(myItems.toArray());
